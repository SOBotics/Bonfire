class ApiKeysController < ApplicationController
  ...
  def destroy
    if @key.destroy
      ...
      flash[:danger] = "An error occurred while deleting the key. Try again later, and if the problem persists, contact a developer."
    end
    redirect_to url_for(:controller => :api_keys, :action => :index)
  end
  ...
  private
    def verify_access
      # Have to do this to allow a user see their own keys; verify_admin would block them.
